...
const arr = [
  ...
  {r: 45, desc: 'AfterDigitalization.first', icon: Icons.DataAnalysisIcon},
  {r: 90, desc: 'AfterDigitalization.second', icon: Icons.DataWorkIcon},
  {r: 135, desc: 'AfterDigitalization.third', icon: Icons.DatabaseIcon},
  {r: 180, desc: 'AfterDigitalization.fourth', icon: Icons.PoliceIcon},
  {r: 225, desc: 'AfterDigitalization.fifth', icon: Icons.NotificationIcon},
  {
    r: 270,
    desc: 'AfterDigitalization.sixth',
    icon: Icons.AtoZTrainLocationIcons
  },
  {r: 315, desc: 'AfterDigitalization.seventh', icon: Icons.MapIcon},
]
...
<template>
  <div class="w-full min-h-full flex flex-column justify-content-center align-items-center">
    ...
    <TitleText :title="$t('AfterDigitalization.title')" class='heading'></TitleText>
    <PulseAnimation class='custom-pulse-animation overflow-visible'/>
    <PulseCenter class='pulse'>
      <div class='h-full relative w-full z-5'>
        <img alt='e-nakl' class='e-nakl absolute cursor-pointer' src='/images/e-nakl-logo.png'
             @click='toggle($event, 5)'>
        <img alt='emm' class='vo-logo absolute cursor-pointer' src='/images/emm-logo.png' @click='toggle($event, 4)'>
        <img alt='virtual-cabinet' class='emm-logo absolute cursor-pointer' src='/images/virtual-cabinet-logo.png'
    ...
    <div class='outline-wrapper'>
      <div class='relative'>
        <div v-for='(item, idx) in arr' :key='item.r' class='thumb-container'>
          <div :style='{transform: `rotate(${item.r}deg)`}' class='seconds'>
            <div :class='`thumb-${idx + 1}`' class='thumb'>
              <component :is="item.icon" v-if="item.icon" style="color: #11A832; width: 50px; height: 50px;"/>
              <img v-else-if="item.img" :src="item.img" alt="checklist" height="70" width="70">
            </div>
            <div :class='`box-${idx + 1}`' class='absolute box'>
              <p>{{ $t(item.desc) }}</p>
            </div>
          </div>
        </div>
